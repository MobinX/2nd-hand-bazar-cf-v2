...
export const Table = ({ datas, labels ,parentUser = null }: { datas: UserType[], labels: string[], parentUser?: {id:number, name: string} |null}) => {
            ...
            <div className='card-body p-0 pb-5'>
                <div className='flex flex-row justify-between items-center w-full px-5 pt-5'>
                    ...
                    <div className='flex items-center gap-3'>
                        ...
                        <Link href={parentUser ? `new?parentId=${parentUser.id}&parentName=${parentUser.name}` : "user/new"}><button className='btn btn-sm btn-outline  btn-square'><PlusIcon className='w-5 h-5 ' /></button></Link>
                        ...
                        <tbody>
                            {datas.filter(d => d.name?.toLowerCase().includes(srchQry.toLowerCase())).map((data, index) => (
                                <tr key={index}>
                                    ...
                                    <td><Link href={`/admin/user/${data.id}`}>{data.name}</Link></td>
                                    <td>{data.email}</td>
